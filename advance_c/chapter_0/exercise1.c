...
//But the searching method is with time complexity of O(n), better use a datastructure of dictionary. To make the time complexity become O(1)
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
...
book *delete_book(book *head, int location, book *deleted)
{
    if (head == NULL || location < 1) return head;
    ...
    if (current == NULL) return head;  // Location out of bounds
